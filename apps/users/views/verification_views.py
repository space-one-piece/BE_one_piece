...
class EmailConfirmView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        if token:
            return Response({"detail": "이메일 인증에 성공했습니다.", "email_token": token}, status=status.HTTP_200_OK)
...
class SmsConfirmView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        if token:
            return Response({"detail": "휴대폰 인증에 성공했습니다.", "phone_token": token}, status=status.HTTP_200_OK)
        return Response({"detail": "인증번호가 잘못되었거나 만료되었습니다."}, status=status.HTTP_400_BAD_REQUEST)
